...
export default defineConfig({
  base: process.env.NODE_ENV === 'production' ? '/booking-suite' : '',
  output: 'server',
  devToolbar: {
    enabled: false,
  },
  ...
  integrations: [
    react(),
    // Only inject dev script in development mode
    ...(process.env.NODE_ENV !== 'production' ? [injectDevScript({scriptPath: '/generated/dev-only.js'})] : []),
  ],
  vite: {
    ...
    build: {
      cssCodeSplit: false, // Evita que se fragmente el CSS de React
      rollupOptions: {
        output: {
          // Asegurar que archivos estáticos se copien bajo la ruta pública real de Cloudflare
          assetFileNames: (assetInfo) => {
            // Mantener sw.js en la raíz
  ...
  // Asegurar que archivos públicos se copien
  publicDir: 'public',
});